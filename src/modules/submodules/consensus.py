...
class ConsensusModule(ABC):
    ...
    def check_contract_configs(self):
        ...
        # todo
        if any((config.genesisTime != genesis_time,
                config.secondsPerSlot != int(cc_config.SECONDS_PER_SLOT),
                config.slotsPerEpoch != int(cc_config.SLOTS_PER_EPOCH))):
    ...
    def get_chain_config(self, blockstamp: BlockStamp) -> ChainConfig:
        ...
        #     ChainConfig,
        # )
        cc = self.w3.lido_contracts.validators_exit_bus_oracle.functions.getChainConfig().call()
        logger.info({'msg': 'Fetch chain config.', 'value': cc})
        return cc
    ...
    def get_blockstamp_for_report(self, last_finalized_blockstamp: BlockStamp) -> Optional[ReferenceBlockStamp]:
        ...
        # Check if contract is currently reportable todo
        ...
        current_epoch_id = (block_stamp.block_timestamp - chain_config.genesisTime) // (chain_config.slotsPerEpoch * chain_config.secondsPerSlot)
        ref_epoch = current_epoch_id // chain_config.epochsPerFrame * chain_config.epochsPerFrame;
        ref_slot = ref_epoch * chain_config.slotsPerEpoch - 1
        ...
        # return (_getTime() - _beaconSpec.genesisTime) / (_beaconSpec.slotsPerEpoch * _beaconSpec.secondsPerSlot); 获取当前时刻所处的 Epoch ID
        # return _epochId / _beaconSpec.epochsPerFrame * _beaconSpec.epochsPerFrame; Epoch 所属的 Frame 的第一个 Epoch ID
        # return epoch * SLOTS_PER_EPOCH-1;  根据epoch计算slot
