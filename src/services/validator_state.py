...
class LidoValidatorStateService:
    ...
    def get_lido_newly_stuck_validators(self, blockstamp: ReferenceBlockStamp, chain_config: ChainConfig) -> dict[
        NodeOperatorGlobalIndex, int]:
    ...
    def get_last_requested_to_exit_pubkeys(self, blockstamp: ReferenceBlockStamp, chain_config: ChainConfig) -> set[
        HexStr]:
    ...
    # 获取每个节点操作者的最后退出验证器的索引
    def get_operators_with_last_exited_validator_indexes(self, blockstamp: BlockStamp) -> dict[
        NodeOperatorGlobalIndex, int]:
        # 获取 Lido 智能合约中的所有节点操作者信息
        node_operators = self.w3.lido_validators.get_lido_node_operators(blockstamp)
        # 获取所有模块信息
        ...
        for module in staking_modules:
            ...
            last_requested_validators = self._get_last_requested_validator_indices(blockstamp, module,
    ...
    def _get_last_requested_validator_indices(self, blockstamp: BlockStamp, module: StakingModule,
                                              node_operators_ids_in_module: Sequence[int]) -> list[int]:
    ...
    def get_recently_requested_but_not_exited_validators(
        ...
        # 获取所有验证者列表
        # validators_validators = registry.functions.getNodeValidators(0, 0).call()[1]
        ...
        # **exiting['args'] 表示将字典 **exiting['args']中的所有键值对解包并添加到当前字典中
        for exiting in exiting_events:
            for validator in dawn_pool_validators:
                # pubkey相等并且未退出
                if exiting['pubkey'] == validator['pubkey'] and (not is_on_exit(validator)):
                    validators_recently_requested_to_exit.append({
                        **exiting['args'],
                        **validator['args'],
                    })
                    break

        # for validator in validators_recently_requested_to_exit:
        #     if is_on_exit(validator):
        #         validators_recently_requested_to_exit.remove(validator)
        ...
        #         delayed_timeout_in_epoch = self.get_validator_delayed_timeout_in_slot(
        #             blockstamp) // chain_config.slots_per_epoch
        #         return is_validator_eligible_to_exit(validator,
        #                                              EpochNumber(blockstamp.ref_epoch - delayed_timeout_in_epoch))
        #
